var app = angular.module('app', [ 'ngCookies', 'ui.router', 'oc.lazyLoad' ]);// ngSanitize
...
.config([ '$ocLazyLoadProvider', function($ocLazyLoadProvider) {
	...
	$ocLazyLoadProvider.config({
		...
		modules : [ {
			...
			name : 'uiFullscreen',
			files : [ './public/js/directives/ui-fullscreen.js' ]
		}, {
			name : 'ui-tree',
			files : [ 'public/vendor/angular/angular-ui-tree/angular-ui-tree.min.js' ]
		}, {
			name : 'ui-menu',
			files : [ './public/js/directives/ui-menu.js' ]
		}, {
			name : 'ui-toggleclass',
			files : [ './public/js/directives/ui-toggleclass.js' ]
		}, {
			name : 'ui-shift',
			files : [ './public/js/directives/ui-shift.js' ]
		},{
			name : 'jQuery',
			files : [ './public/vendor/jquery/jquery.min.js' ]
		} ]
	})
} ])
// 配置路由相应
.config(function($stateProvider, $urlRouterProvider) {
	...
	$stateProvider.state('access', {
		...
		resolve : {
			deps : [ '$ocLazyLoad', function($ocLazyLoad) {
				return $ocLazyLoad.load([ 'ui-bootstrap', 'bootstrap.js', 'uiFullscreen', 'ui-menu', 'jQuery', 'ui-toggleclass', 'ui-shift' ]);
			} ]
		}
	}).state('app.index', {
		url : '/index',
		...
		template : '<div ui-view class="fade-in-right-big smooth"></div>',
	/*
	 * resolve : { deps : [ '$ocLazyLoad', function($ocLazyLoad) { return $ocLazyLoad.load([ 'content/article.js' ]); } ] }
	 */
	}).state('app.article.read', {
		url : '/{menuId:[0-9]+}',
		templateUrl : './content/article.html',
		resolve : {
			deps : [ '$ocLazyLoad', function($ocLazyLoad) {
				return $ocLazyLoad.load(['content/article.js' ]);
			} ]
		}
	})
...
.controller('AppCtrl', [ '$scope', '$window', '$cookies', function($scope, $window, $cookies) {
	...
	$scope.app = {
		...
		user :  $cookies || $cookies.getObject('user')||{}
	}
