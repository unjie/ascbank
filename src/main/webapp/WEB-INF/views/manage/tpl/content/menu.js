app.controller('MenuCtrl', [ '$scope', '$http', '$timeout', '$uibModal', function($scope, $http, $timeout, $uibModal) {
	$scope.data = [];
	...
	$scope.removeSubItem = function(scope) {
		...
		if (!(nodeData.id) && !(nodeData.stem)) {
			...
			$http.get("./menu/destroy/stem/" + nodeData.stem + nodeData.id + ',').success(function(largeLoad) {
				if (largeLoad.success) {
					$http({
						url : "./menu/destroy/" + nodeData.id,
						method : 'DELETE'
	...
	var newMenu = function(nodeData) {
		return {
			...
			'stem' : nodeData.stem + ((nodeData.id === null) ? '' : nodeData.id) + ',',
			'children' : [],
			'alias' : null,
			'author' : null,
			'clicks' : 0,
	...
	var menuSave = function(data) {
		...
		$http(param).success(function(largeLoad) {
			...
			if (largeLoad.success && (!!largeLoad.data) && (!!largeLoad.data.id)) {
	...
	var articleSave = function(data) {
		...
		$http(param).success(function(largeLoad) {
			if (largeLoad.success && (!!largeLoad.data) && (!!largeLoad.data.id)) {
				$scope.menuData.article.id = largeLoad.data.id;
			}
		});
	}
	...
	var getArticle = function(nodeData, fun) {
		if (typeof nodeData.article === 'undefined') {
			$http.get('./article/read/menuid/' + nodeData.id).success(function(largeLoad) {
				if (largeLoad.success) {
					...
					console.log('Error', '获取Article 失败');
				}
	...
	$scope.editorArticle = function(nodeData) {
		...
		getArticle(nodeData, function(data) {
			console.log(data);
			$scope.articleData = data.article;
			$uibModal.open({
				size : 'lg m-n no-padder',// aside
				templateUrl : './manage/tpl/content/articleFrom.html',
				scope : $scope
			});
